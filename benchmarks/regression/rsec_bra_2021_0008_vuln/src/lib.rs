...
//! `GreedyBufReader` and calling its `BufRead::fill_buf` implementation.
...
impl Read for ObservingRead {
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        // In bra 0.1.0, GreedyBufReader::fill_buf passed an uninitialized
...
pub fn run_public_api_poc() -> ReadObservation {
    ...
    let mut reader = GreedyBufReader::with_capacity(inner, 16);
...
mod tests {
    ...
    fn rustdpr_deterministic_trace_replay() {
        ...
        assert!(observation.read_calls > 0, "GreedyBufReader::fill_buf should read from the wrapped reader");
    }
}
