...
def main() -> int:
    parser = argparse.ArgumentParser(description="Normalize FourFuzz/FourFuzz-style outputs")
    ...
    parser.add_argument("--original", action="store_true", help="label as original FourFuzz instead of approximation")
    ...
    unsafe_hits_path = raw_dir / "unsafe_hits.json"
    unsafe_hit_count = 0
    if unsafe_hits_path.exists():
        data = json.loads(unsafe_hits_path.read_text(encoding="utf-8"))
        unsafe_hit_count = len(data if isinstance(data, list) else data.get("hits", []))

    tool = "fourfuzz" if args.original else "fourfuzz-style"
    notes = (
        "original FourFuzz output normalized for RustDPR validation"
        if args.original
        else "FourFuzz-style approximation; not original FourFuzz implementation"
    )
